...
def get_next_word_packet(s):
    ...
    while True:
        if len(packet_buffer) >= WORD_LEN_SIZE:
            ...
            total_len = WORD_LEN_SIZE + word_len

            if len(packet_buffer) >= total_len:
                word_packet = packet_buffer[total_len:]
                packet_buffer = packet_buffer[total_len:]
                return word_packet
